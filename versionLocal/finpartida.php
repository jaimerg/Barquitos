...
while ($registro = mysqli_fetch_array($consulta)) {
    if ($registro['id'] == $_SESSION['ganador']) {
        ...
        $sql = mysqli_query($conexion,"update usuarios set estado='".$estado."', peticion='".$peticion."', tablero='' where nick='".$usuario."'");
...
while ($registro = mysqli_fetch_array($consulta2)) {
    if ($registro['id'] == $id_peticion) {
        ...
        $sql = mysqli_query($conexion,"update usuarios set estado='".$estado."', peticion='".$peticion."', tablero='' where id='".$id_peticion."'");
    } 
}
